...
\title{Transformers and Canonical Correlation Analysis for  Causal Inference}
\author{IA pour la Science}
\date{January $3^\text{th}$, 2026}
...
\end{frame}
%=============================================
\begin{frame}{Connected and disconnected causality}
...
\begin{frame}{Interpretation for Time Series and CCM}
    ...
    %One method suggested by (Martin and Maes 1979)
    Let $U, S, V\T$ be the
    %Singular Value Decomposition (\textit{SVD})
    SVD of the matrix $Z = S_{11}^{- \frac{1}{2}} S_{12} S_{22}^{- \frac{1}{2}}$. Then $A^*, B^*$ and the total maximum canonical correlation are
...
%=============================================
\begin{frame}{Self-attention and cross-attention (1)}
