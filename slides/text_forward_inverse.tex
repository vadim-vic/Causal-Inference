...
\draw[arrow] (wt) --  node[above]{{fine tune}} (wtm1);
...
\draw[arrow] (xt) -- node[above] {$\mathcal{L}, \beps$} (xtm1);

% Vertical arrows (forward operators)
\draw[arrow] (xt) -- node[left] {$\eta$} (wt);
\draw[arrow, bend right=40] (xt) to node[left] {\raisebox{-12ex}{$\bA_t^\prime$}} (zt);
\draw[arrow] (xtm1) -- node[right] {$\eta$} (wtm1);
\draw[arrow, bend right=40] (xtm1) to node[right] {\raisebox{-12ex}{$\bA_{t-1}^\prime$}} (ztm1);
...
\begin{enumerate}
\item 
\item Fine-tune the SCM parameters $\hat{\bw}_t = \arg\min \loss(\bw \mid \hat{\bw}_{t-1})$ with the initial parameters from the step $t-1$.
...
\item Sample the next adjacency $\bz_{t-1}$.
\end{enumerate}
