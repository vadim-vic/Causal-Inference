...
\subsection*{A basic diffusion process for the binary adjacent $\bz$}
...
\node (xt) [right=of dots1] {$x_t$};
\node (xtm1) [right=of xt] {$x_{t-1}$};
\node (dots2) [right=of xtm1, draw=none] {$\cdots$};
\node (x0) [right=of dots2] {$x_0$};

% Forward arrows (top)
\draw[arrow] (xT) -- (dots1);
\draw[arrow] (dots1) -- (xt);
\draw[arrow] (xt) -- node[above] {$p_\theta(x_{t-1} \mid x_t)$} (xtm1);
\draw[arrow] (xtm1) -- (dots2);
\draw[arrow] (dots2) -- (x0);

% Reverse / diffusion process (bottom dashed)
\draw[dashedarrow, bend right=40] (xtm1) to node[below] {$q(x_t \mid x_{t-1})$} (xt);
\draw[dashedarrow, bend right=60] (xtm1) to node[below] {$q(x_t \mid x_{t-1})$} (xt);

% Unknown reverse
%\node[draw=none, below=.5cm of xt] (unknown)  {\color{red} $q(x_{t-1} \mid x_t)$ is unknown};
\end{tikzpicture}
...
% Middle variables
\node[var, below=of zt] (wt) {$\bw_t$};
\node[var, below=of ztm1] (wtm1) {$\bw_{t-1}$};
...
% Bottom variables
\node[var, below=of wt] (xt) {$\bx_t$};
\node[var, below=of wtm1] (xtm1) {$\bx_{t-1}$};
...
\draw[arrow, bend right=40] (xtm1) to node[right] {\raisebox{4.5ex}{$\bA_{t-2}^\prime$}} (ztm1);
